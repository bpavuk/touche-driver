...
#[cfg(target_os = "linux")]
pub struct GraphicsTabletDevice {
    device: Option<VirtualDevice>,
}

#[cfg(target_os = "linux")]
impl GraphicsTabletDevice {
    pub fn new_uninit() -> GraphicsTabletDevice {
        GraphicsTabletDevice { device: None }
    }
}

impl DeviceSink for GraphicsTabletDevice {
    ...
    fn init(&mut self, width: i32, height: i32) -> Result<(), Box<dyn std::error::Error>> {
        info!("device setup. width {} height {}", width, height);
